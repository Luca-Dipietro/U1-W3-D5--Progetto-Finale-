...
const me = { name: "Luca", surname: "Dipietro", age: "26" };
console.log(me);
...
me.skills = ["Java Script", "CSS", "HTML"];
console.log(me);
...
console.log(deleteOne("Epicode School", true));
...
const onlyLetters = function (stringa) {
  const letters = stringa.split("").filter((str) => {
    if (!(str >= "0" && str <= "9")) {
      return true;
    }
  });
...
console.log(onlyLetters("I have 14 dogs"));
...
console.log(isThisAnEmail("luca.dipietro.ld@gmail.com"));
...
const rollTheDices = function (n) {
  ...
  for (let i = 0; i < n; i++) {
    const numRolled = dice();
    arrayOfValues.push(numRolled);
    sum += numRolled;
  }

  return { sum: sum, values: arrayOfValues };
};
...
const howManyDays = function (date) {
  const currentDay = new Date();
  const datePar = new Date(date);
  const currentDayInMS = currentDay.getTime();
  const dateInMS = datePar.getTime();
  const diffMSDays = currentDayInMS - dateInMS;
  const diffDays = Math.floor(diffMSDays / (1000 * 60 * 60 * 24));
  return diffDays;
};
console.log(howManyDays("2020-04-15"));
...
const obj = { name: "Luca", age: "26", student: true };
const str = "student";
console.log(deleteProp(obj, str));
...
const newestMovie = function (array) {
  ...
  array.forEach((movie) => {
    if (parseInt(movie.Year) > parseInt(newestFilm.Year)) {
      newestFilm = movie;
    }
  });
  return newestFilm;
...
const searchByTitle = function (str, array) {
  const result = [];
...
const searchAndDivide = function (str, array) {
  const match = [];
  const unmatch = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].Title.toLowerCase().includes(str.toLowerCase())) {
...
const removeIndex = function (n, array) {
